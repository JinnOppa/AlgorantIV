...
#    along with ibathy.py.  If not, see <http://www.gnu.org/licenses/>.
...
class Gridinfo(object):
    ...
    def __init__(self, filename, ig):
        ...
        if ig == 0:
            ...
            # print(gh.dimensions)
            nlons = gh.dimensions['COLUMNS'].size
            nlats = gh.dimensions['LINES'].size
            dlev = 0
            self._nlons = int(nlons)
            self._nlats = int(nlats)
            self._nlevs = int(dlev)
            off = gh["DEPTH_SMOOTH"].add_offset
            mv=gh["DEPTH_SMOOTH"].ext_missing_value
            sf = gh["DEPTH_SMOOTH"].scale_factor
            lats = np.array(gh["LINES"])
            lons = np.array(gh["COLUMNS"])
            dep = gh["DEPTH_SMOOTH"]
            # np.where(dep == mv, 0., dep)
...
class Sbathy(object):
    ...
    def __init__(self, filename, inc, *args,checkb=0):
        ...
        for g in args:
            ...
            # check bounds -----------------------------
            #i = pn[0]
            #j = min(pn[1], gmed.nlons - 1)
            #print(gmed.lons[i], gmed.lons[j])
            #i = pn[2]
            #j = min(pn[3], gmed.nlats - 1)
            #print(gmed.lats[i], gmed.lats[j])
            # -----------------------------
    ...
    def get_ibox(self, gmed, gemo):
        ...
        fmed = gmed.box.p1.plon  # start of med bathy long.
        ...
        if fmed < xmod or xmed > fmod:  # no intersection #check:
            ...
            if xmed < fmod:
                if xmed <= xmod:
                    ...
                    i = 0
                    j = (fmod - xmed) * gmed.nresx+1#Ale
                    res1 = 0
                    res2 = min(int(j), gmed.nlons)

        fmed = gmed.box.p1.plat  # start of med bathy long.
        xmed = gmed.box.p0.plat  # start of med bathy
        xmod = gemo.box.p0.plat  # start of emodnet block long
        fmod = gemo.box.p1.plat  # end of emodnet block long
